...
class ItemTextParser() {
  def parseItemText(itemText: String): Option[Map[String, String]] = {
    def wrapInOption(items: Map[String, String]): Option[Map[String, String]] = {
      if (items.isEmpty) None else Some(items)
    }

    val items = convertItemTextToMap(Map[String, String](), itemText)
    wrapInOption(items)
  }

  def convertItemTextToMap(items: Map[String, String], itemText: String): Map[String, String] = {
...
object ItemTextParser extends App {
  ...
  // val itemText = "<$%$TITLE>Job Title<$%$AUSNZ$%$>TURE"
  ...
  for (
    (k, v) <- itemTextParser.parseItemText(itemText).get
