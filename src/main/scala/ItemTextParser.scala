...
object ItemTextParser extends App {
  ...
  // val itemText = "<$%$TITLE>Job Title<$%$AUSNZ$%$>TURE"
   val itemText = "<$%$DESCRIPTIONTITLE$%$>some descripton<$%$TITLE>Job Title"
  ...
  val parsedItemText = itemTextParser.mapItemText(itemText)
  if (parsedItemText != null)
    for ((k, v) <- parsedItemText) println(s"(key, value): (${k}, ${v})")
  else
    println("null")
}
